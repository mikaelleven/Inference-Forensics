    ...
    def run(
        ...
        *,
        ...
        if explicit_workdir is not None and not explicit_workdir.is_dir():
            raise NotADirectoryError(f"Work directory was not found or is not a directory: {explicit_workdir}")

        with tempfile.TemporaryDirectory(prefix="codex-run-") as temp_dir:
            temp_root = Path(temp_dir)
            codex_home = temp_root / "codex-home"
            temporary_workdir = temp_root / "workdir"
            codex_home.mkdir()
            temporary_workdir.mkdir()
            workdir_path = explicit_workdir or temporary_workdir
