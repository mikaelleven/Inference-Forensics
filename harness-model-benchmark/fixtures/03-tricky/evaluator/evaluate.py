...
def main() -> int:
    ...
    visible_module = load_module(
        "fixture_visible_tests",
        workspace / "tests" / "test_policy_engine.py",
    )
    hidden_module = load_module(
        "fixture_hidden_tests",
        evaluator_dir / "hidden_tests.py",
    )

    visible_result, visible_output = run_suite(visible_module)
    hidden_result, hidden_output = run_suite(hidden_module)

    protected_relpaths = [Path("tests/test_policy_engine.py")]
    changed_protected = []
    for relative in protected_relpaths:
    ...
    # Modifying protected tests invalidates the benchmark regardless of test outcome.
    score = test_score if protected_unchanged else 0.0
    passed = (
        visible_result.wasSuccessful()
        and hidden_result.wasSuccessful()
    ...
    result = {
        ...
        "visible": {
            "passed": passed_count(visible_result),
            "total": visible_result.testsRun,
            "ok": visible_result.wasSuccessful(),
        },
        "hidden": {
            "passed": passed_count(hidden_result),
            "total": hidden_result.testsRun,
            "ok": hidden_result.wasSuccessful(),
        },
